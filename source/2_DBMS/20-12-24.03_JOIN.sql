...
SELECT * FROM DEPT;
-- 02. CROSS JOIN(FROM절에 테이블 2개 이상)
...
-- 테이블 별명(별칭)을 사용할 경우 제일 먼저 적용, 전부 별명으로 적어야 함(본래 이름으로 사용 불가)
-- 필드 별명은 WHERE절에서는 사용 불가, 그 외 절에서는 별명도 본래 이름도 사용 가능

-- EMP 테이블과 DEPT 테이블의 모든 필드(부서번호는 한 번만 나오게)
...
-- 급여 2000이상인 직원만 이름, 업무, 급여, 부서명, 근무지 필드
...
SELECT WORKER.EMPNO, WORKER.ENAME, MANAGER.ENAME
    ...
    WHERE WORKER.MGR=MANAGER.EMPNO; -- EMPNO에 NULL값이 없으므로 13명 출력(MGR이 NULL인 사람:KING은 출력X)
                                    -- OUTER JOIN 이용하면 KING 출력 가능(조인 조건 만족하지 못한 행도 출력 가능)
-- "SMITH의 상사는 FORD다" 스타일로 출력
SELECT W.ENAME||'의 상사는 '||M.ENAME||'다' 
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO; --EMPNO에 NULL값이 없으므로 13명 출력
-- SELF JOIN 연습문제
-- 매니저가 KING인 사원들의 이름과 직급을 출력하시오. --SELF
SELECT W.ENAME, W.JOB 
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO AND M.ENAME='KING';
-- SCOTT과 동일한 부서번호에서 근무하는 사원의 이름을 출력하시오 --SELF
SELECT E2.ENAME
    FROM EMP E1, EMP E2
    WHERE E1.DEPTNO=E2.DEPTNO AND E1.ENAME='SCOTT' AND E2.ENAME<>'SCOTT'; 
-- SCOTT과 동일한 근무지에서 근무하는 사원의 이름을 출력하시오(2단계 최종문제) --SELF, EQUI JOIN
SELECT E2.ENAME
...
-- ★06. OUTER JOIN; 조인 조건에 만족하지 않은 행까지 나타나게 
SELECT W.EMPNO, W.ENAME, M.ENAME
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO(+); --조건 만족하지 않은 쪽에 (+)추가
SELECT W.EMPNO, W.ENAME, NVL(M.ENAME, '-CEO-') --NVL함수 이용
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO(+);
SELECT W.EMPNO, W.ENAME, M.ENAME
    FROM EMP W, EMP M
    WHERE W.MGR(+)=M.EMPNO AND W.EMPNO IS NULL; --말단 사원(부하 직원이 없는 사원) 출력
SELECT * FROM EMP; --14행
SELECT * FROM DEPT; --4행
SELECT E.*, DNAME FROM EMP E, DEPT D
    WHERE E.DEPTNO(+)=D.DEPTNO; --(+)는 조건 만족하지 않은(EX: NULL값 가진)쪽 에 붙임
    
-- OUTER JOIN 연습문제
--FORD의 매니저는 JONES입니다. (KING의 매니저는 無입니다.)
...
SELECT E1.ENAME 사원명, NVL(E2.ENAME, '(NONE)') 상사
    FROM EMP E1, EMP E2
    WHERE E1.MGR=E2.EMPNO(+);
--10. 사원명, 상사명, 상사의 상사명을 검색하시오
SELECT E1.ENAME 사원명, E2.ENAME 상사명, E3.ENAME "상사의 상사명"
